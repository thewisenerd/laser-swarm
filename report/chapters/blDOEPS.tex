...
For satellites using solar arrays the interface between the arrays and the bus also needs to be controlled. There are two ways of doing this: with a peak-power tracker (PPT) and with direct-energy-transfer (DET). 
...
A power distribution system (PDS) consists of the cabling, fault protection and on/off switching gear. The first choice to make is the type of current of the distribution system. Mostly direct current is used because spacecraft generate direct current. An AC/DC converter would need more electronics, resulting in more parts and a higher mass. 
