...
public class FindElevation {
	 ...
	 */
	public double findSNR(final DataContainer data, TimePair timeRng) {
		...
		for (NoiseData noiseIt : nsData) {  	//iterate over the interpulsewindows
			tNoise += noiseIt.getNoiseFrameL().diff()+	//calculate time of the noise in one interpulsewindow
			noiseIt.getNoiseFrameR().diff();
			tSignal += noiseIt.getWindowFrame().diff();	//calculate the signal time in one interpulse window
		...
		return signal/tSignal + noise/tNoise; 
	...
	public static double calcAlt( Point3d emit, Point3d rec, double travTime) throws MathException {
		...
		double focalDist = emit.distance(rec); // distance between the focal points formed by receiver and emitter 
		double c = Configuration.c; // speed of light
		double dist = Math.abs(travTime) * c;
		if (dist < focalDist)throw new MathException("Distance Traveled is shorter than Focal length");
		
		double a = dist / 2; // semimajor axis

		double b_2 = Math.pow(dist / 2, 2) - Math.pow(focalDist / 2.0, 2.0); // b^2 (semiminor axis squared) 
		...
		if(dif.length()==0) theta = Math.PI/2; //if the receiver and emitter are the same
			
		double distGrndEmit = a * (1 - ecc_2) / (1 - ecc * Math.cos(theta)); // distance to the ground from the emitter
		
		return em.length() - Configuration.R0- distGrndEmit; // altitude above the earth sphere in meters
