...
public class BRDF extends Distribution  {	
	 ...
	 * @param otSl
	 * @param trDir in ENU
	 * @param vector the actual vector in ENU to convert
	...
	private void makeRot(double atSl, double otSl, Vector3d trDir) {
		...
		Vector3D atProj = new Vector3D(trDir.x, trDir.y, 0.0);	//projected trDir vector onto the ENU plane
	...
	//	Rotation alignX  = new Rotation(Vector3D.PLUS_J, Math.atan(atSl)); 		//rotate around the yaxis by the slope angle
	//	Rotation alignY  = new Rotation();
		
		
		Vector3D sX = new Vector3D(atProj.getX(), atProj.getY(), atSl); 		//vector  that represents the slanted plane in the ENU
		sX=sX.normalize();
		logger.inf("sX: %s", sX);
		Vector3D sY = new Vector3D(otProj.getX(), otProj.getY(), otSl);			//vector that represents the y -axis of the lsanted plane in ENU coords
		sY=sY.normalize();
		logger.inf("sY: %s", sY);
																			//Vector3D proj = new Vector3D(trDir.x, trDir.y, 0.0);
	//	Rotation rot1 = new Rotation(sX, Vector3D.PLUS_I);  				//rotation that transfers the sX-axis to the ENU x-axis
		...
		Rotation alignNorm = new Rotation(Vector3D.PLUS_K,snorm);	// rotation that transforms the normals from one plane to another
		//Rotation align
		
		Vector3D xaxisS = alignNorm.applyTo(Vector3D.PLUS_I);		// transform the x- axis vector to the coordinates of the slanted plane
