\subsection{\ac{laser} Focus Calculation}
\label{focus}
In order to diverge or focus the laser beam, it is possible to move the parabolic mirror up or down from the exact focus position. In this case, the divergent angle $\gamma$ can be calculate, which can be verified or optimized later on to obtain desired footprint size. The calculation drawing is shown in figure \ref{fig:focus} on page \ref{fig:focus}.
...
\begin{equation}
\label{miu2}
(\frac{dy}{dx})_{x_{1},y_{1}} = \frac{y_{1}+p}{x_{1}} \Rightarrow \mu_{1} = atan(\frac{y_{1}+p}{x_{1}}) \Rightarrow \mu_{2} = 90deg - \mu_{1} - \mu_{3}
\end {equation}
